...
- ✅ **Phase 2.5** MNIST Box dataset ([plan](plans/mnist_box_plan.md))
  ...
  - ✅ (bonus) **e2 2×2 ablation: prior × interp space** — `state N(0,I)` vs `arb clip(N(0.5,1/6²),0.02,1) cxcywh` prior × `state` vs `cxcywh` interp. 반전 결과: 지배 요인은 **interp 공간** 뿐이고, prior 는 부차적. `riemannian_arb_prior` (arb prior + state interp) 가 **baseline riemannian 보다 mean_err 2× 개선** (2.6 vs 5.6 px) — prior 가 target 에 가까우면 state interp 가 더 잘 먹힘. `euclidean_arb_prior` 의 나쁜 성능은 entropy 부족이 아니라 cxcywh interp 의 per-dim 신호 불균형 때문. 추가 artefact: `RiemannianTrajectoryArbPrior` 클래스, 4-panel GIF. [`experiments/e2_arbitrary_euclidean_prior/report.md`](../experiments/e2_arbitrary_euclidean_prior/report.md)
